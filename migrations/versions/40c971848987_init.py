...
Revision ID: 40c971848987
Revises: 
Create Date: 2026-01-11 13:07:23.969482
...
def upgrade() -> None:
    ...
    sa.Column('triggeredOnLIVE', sa.Integer(), nullable=False),
    sa.Column('triggeredOnQA', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['experimentId'], ['experiment.id'], ),
    sa.ForeignKeyConstraint(['metricId'], ['metrics.id'], ),
    sa.ForeignKeyConstraint(['variationId'], ['variation.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('metricId', 'variationId', 'experimentId', name='unique_metric_variation_experiment_result')
    )
    op.create_index(op.f('ix_result_experimentId'), 'result', ['experimentId'], unique=False)
    op.create_index(op.f('ix_result_metricId'), 'result', ['metricId'], unique=False)
    op.create_index(op.f('ix_result_variationId'), 'result', ['variationId'], unique=False)
    # ### end Alembic commands ###

def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index(op.f('ix_result_variationId'), table_name='result')
    op.drop_index(op.f('ix_result_metricId'), table_name='result')
    op.drop_index(op.f('ix_result_experimentId'), table_name='result')
